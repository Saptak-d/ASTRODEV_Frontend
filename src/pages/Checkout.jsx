...
export default function Checkout() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0F0A06] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-[#9A8B7A] text-xs tracking-[0.3em] uppercase">Preparing Checkout…</p>
      </div>
    );
  }
  ...
  if (error && !orderData) {
    return (
      <div className="min-h-screen bg-[#0F0A06] flex flex-col items-center justify-center gap-5 px-6 text-center">
        <div className="text-5xl">⚠️</div>
        <h2 className="text-xl font-bold text-[#F5F2E9] tracking-widest uppercase">Checkout Error</h2>
        <p className="text-[#9A8B7A] text-sm max-w-sm">{error}</p>
        <button
          onClick={() => navigate('/generate')}
          className="mt-2 px-8 py-3 bg-[#D4AF37] text-[#0F0A06] font-bold rounded-xl text-sm uppercase tracking-wider hover:brightness-110 transition-all"
        >
          Go Back
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0F0A06] flex flex-col items-center justify-center px-4 py-10">
      <style>{`
        ...
        @keyframes borderGlow{0%,100%{box-shadow:0 0 12px rgba(212,175,55,.15)}50%{box-shadow:0 0 30px rgba(212,175,55,.4)}}
      ...
      <div className="fu1 text-center mb-8">
        ...
        <p className="text-[#9A8B7A] text-[10px] tracking-[0.3em] uppercase">
      ...
      <div className="w-full max-w-md space-y-4">
        ...
        {/* Order Summary Card */}
        <div className="fu2 card-glow bg-[#1A1108] border border-[#D4AF37]/25 rounded-2xl overflow-hidden">
          <div className="h-[2px] bg-gradient-to-r from-transparent via-[#D4AF37] to-transparent" />
          <div className="px-6 py-5">
            <p className="text-[9px] tracking-[0.35em] text-[#D4AF37]/70 uppercase font-bold mb-4">
              📋 Order Summary
            </p>

            {/* Name */}
            <div className="text-center bg-[#0F0A06] rounded-xl py-3 px-4 mb-4 border border-[#D4AF37]/10">
              <p className="text-[8px] text-[#D4AF37] tracking-[0.2em] uppercase font-bold mb-0.5">Name · नाम</p>
              <p className="text-lg font-extrabold text-[#F5F2E9] tracking-wide">{displayName}</p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center mb-4">
              {[
                ...
                <div key={d.en} className="bg-[#0F0A06] rounded-xl p-2.5 border border-[#D4AF37]/10">
                  <span className="text-lg">{d.icon}</span>
                  <p className="text-[7px] text-[#D4AF37]/70 uppercase tracking-wider font-bold mt-1">{d.en}</p>
            ...
            <div className="flex items-center justify-between border-t border-[#D4AF37]/10 pt-4">
              <div>
                <p className="text-[#9A8B7A] text-[10px] uppercase tracking-widest font-semibold">Personalized Kundli Report</p>
                <p className="text-[#C9B99A] text-[9px] mt-0.5">Hindi PDF · WhatsApp + Email delivery</p>
              </div>
              <div className="text-right">
                <p className="text-2xl font-extrabold text-[#D4AF37]">{AMOUNT_DISPLAY}</p>
                <p className="text-[#9A8B7A] text-[9px]">incl. taxes</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* What You Get */}
        <div className="fu3 bg-[#1A1108] border border-[#D4AF37]/15 rounded-2xl px-6 py-4">
          <p className="text-[9px] tracking-[0.3em] text-[#D4AF37]/70 uppercase font-bold mb-3">✦ What You Get</p>
          <ul className="space-y-2">
            {[
              ...
              <li key={item} className="flex items-start gap-2 text-[#C9B99A] text-[10px]">
        ...
        {/* Error display */}
        {error && (
          <div className="fu3 bg-red-950/50 border border-red-500/30 rounded-xl px-4 py-3 text-red-300 text-xs flex gap-2 items-start">
        ...
        <div className="fu4">
          <button
            ...
            className="relative w-full flex items-center justify-center gap-2 rounded-2xl py-4 font-extrabold text-[#0F0A06] uppercase tracking-widest text-sm transition-all duration-200 hover:scale-[1.02] active:scale-[0.97] disabled:opacity-60 disabled:cursor-not-allowed overflow-hidden shadow-lg"
          >
            {paying ? (
          ...
          <div className="flex items-center justify-center gap-3 mt-3">
            ...
            <p className="text-[#9A8B7A] text-[9px] tracking-wider uppercase">
        ...
        {/* Trust */}
        <p className="fu4 text-center text-[#9A8B7A] text-[8px] tracking-wider uppercase pb-2">
